...
static bool get_mininginfo(CURL *curl, struct work *work)
{
	...
	if (!val && curl_err == -1) {
		...
		if (res) {
			...
			if (key && json_is_real(key)) {
				net_hashrate = (double) json_real_value(key) * 1e6;
			}
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (jsonrpc_2) {
		...
		if (opt_algo == ALGO_DECRED) {
			...
			// extradata
			for (i = 0; i < (int)(sctx->xnonce1_size/4); i++)
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		if (diff.tv_usec || diff.tv_sec) {
			pthread_mutex_lock(&stats_lock);
			double a = (diff.tv_sec + diff.tv_usec * 1e-6);
			thr_hashrates[thr_id] =  hashes_done / a;
			pthread_mutex_unlock(&stats_lock);
		}
		if (!opt_quiet && (time(NULL) - tm_rate_log) > opt_maxlograte) {
			...
			default:{
				char str[128] = { 0 };
				format_hashrate(thr_hashrates[thr_id], str);				
				#ifdef USE_OPENCL
				if(use_opencl){
				...
				#endif
				{
					applog(LOG_INFO, "%s #%d: %s", "CPU", thr_id, str);
				}
				break;
				}
			}
...
{
	printf("Original\n");
	printf("** " "cpuminer-multi" " " "1.3.4" " by tpruvot@github **\n");
	...
	printf(" config features:"
		...
		" x86_64"
#endif
#if defined(USE_ASM) && (defined(__i386__) || defined(__x86_64__)) && defined(__SSE2__)
		" SSE2"
#endif
#if defined(__x86_64__) && defined(USE_XOP) && defined(__XOP__)
		" XOP"
#endif
#if defined(__x86_64__) && defined(USE_AVX) && defined(__AVX__)
		" AVX"
#endif
#if defined(__x86_64__) && defined(USE_AVX2) && defined(__AVX2__)
		" AVX2"
#endif
...
void parse_arg(int key, char *arg)
{
	...
	SPLIT:
		*p2 = 1;
		...
		if(strlen(arg) < 1) show_usage_and_exit(1);
	...
	if(opt_algo == ALGO_VIPSTAR_CL) {
		...
		for(int i = 0; i < opt_n_vector_width; i++)
			if(vector_width_params[i] != 1 && vector_width_params[i] > 16 && (vector_width_params[i] % 2 != 0 || vector_width_params[i] < 0))
				vector_width_params[i] = 0;

		for(int i = 0; i < opt_n_intensity; i++)
			if(intensity_params[i] != 1 && intensity_params[i] > 16 && (intensity_params[i] % 2 != 0 || intensity_params[i] < 0))
				intensity_params[i] = 0;
...
LOOP_END:;
		...
		for(int i = 0; i < opt_n_threads; i++){
			...
			if(cl[i].device == NULL){
				applog(LOG_ERR, "Unknown error occurred.\n");
				exit(1);
			}
